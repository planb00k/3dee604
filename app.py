...
if run_process and uploaded_file:
    ...
    minima_hist = np.array([i for i in zc_hist if deriv_hist[i - 1] < 0 and deriv_hist[i + 1] > 0]).astype(int) + low_bound
    ...
    for i in range(nom_of_objects):
        ...
        cv2.putText(temp, f"Width {int(x_mm)}mm", (tl_p[0], br_p[1] + 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 3)
        cv2.putText(temp, f"Length {int(y_mm)}mm", (tl_p[0], tl_p[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 3)

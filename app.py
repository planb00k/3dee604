...
if run_process and uploaded_file:
    ...
    # True Difference of Gaussians: two Gaussian smoothings with different sigma
    g1 = gaussian_filter1d(hist, sigma=1.0)
    g2 = gaussian_filter1d(hist, sigma=3.0)
    dog = g1 - g2  # Difference of Gaussians

    # keep a smoothed histogram for reference/plots
    smoothed_hist = gaussian_filter1d(hist, sigma=1.89)

    # detect minima on the DoG signal (zero-crossings where DoG goes from negative to positive)
    zero_crossings = np.where(np.diff(np.sign(dog)))[0]
    minima = np.array([i for i in zero_crossings if (i-1)>=0 and (i+1)<len(dog) and dog[i - 1] < 0 and dog[i + 1] > 0]).astype(int)

    # if minima is empty fallback to a simple heuristic (prevents crash)
    if minima.size == 0:
        # pick some low points from smoothed histogram as fallback
        minima = np.array([np.argmin(smoothed_hist)])

    # ---------------- KMeans Segmentation ----------------
    kmeans = KMeans(n_clusters=nom_of_objects, random_state=42)
    # need at least one sample for kmeans
    if minima.reshape(-1,1).shape[0] >= nom_of_objects:
        kmeans.fit(minima.reshape(-1, 1))
        centers = np.sort(kmeans.cluster_centers_.reshape(-1))
    else:
        # fallback: choose evenly spaced thresholds
        centers = np.linspace(np.min(minima), np.max(minima) if minima.size>0 else 128, num=nom_of_objects)
    ...
    ground_truth = minima[0] if minima.size>0 else 128
    ...
    # ---------------- Measurement Functions (restored original behavior) ----------------
    def sad(camheight, depthmap, mask):
        # find good feature corners on mask
        corners = cv2.goodFeaturesToTrack(mask, 10, 0.05, 50)
        if corners is None:
            # fallback bounding box if corners fail
    ...
    # ---------------- Measurement and Annotation (restored) ----------------
    temp = depth_color.copy()
    bounding_boxes = []
    results = []

    for i in range(nom_of_objects):
        mask_i = (masks[i] if i in masks else np.zeros_like(gray))
        dx, dy, tl_p, br_p = sad(camheight=camh, depthmap=temp, mask=mask_i)
        # convert to int tuples
        tl_p = (int(tl_p[0]), int(tl_p[1]))
        br_p = (int(br_p[0]), int(br_p[1]))
        ...
        # add to results (width, length); depth added later
        results.append({"Object": i + 1, "Width (mm)": int(x_mm), "Length (mm)": int(y_mm)})
        # put width label under bounding box, and vertical length label
    ...
    # compute depth scaling and depth per object (same logic as before)
    ...
    for i in range(nom_of_objects):
        ...
        cv2.putText(temp, f"Depth {int(temph)}mm", bounding_boxes[i][0], cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255,255,0), 2)
        results[i]["Depth (mm)"] = int(temph)

    # ---------------- Helper functions (HTML centering) ----------------
    ...
    # final annotated image (temp) - shows width, length, depth annotations
    ...
    # bounding boxes only (before annotations) for clarity
    ...
    with st.expander("Histogram and DoG Analysis", expanded=False):
        ...
        # DoG visualization and minima marking
        ...
        # minima on smoothed histogram for clarity
        fig_min, ax_min = plt.subplots(figsize=(6, 3))
        ax_min.plot(smoothed_hist, color='black', linewidth=2)
        if minima.size > 0:
            ax_min.scatter(minima, smoothed_hist[minima], color='red', s=40, label="Detected Minima (mapped)")
            for idx, m in enumerate(minima):
                ax_min.text(m, smoothed_hist[m] + max(smoothed_hist)*0.03, f"{idx+1}", color='red', ha='center', fontsize=9)
